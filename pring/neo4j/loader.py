...
def _sanitize_node_record(node: Dict[str, Any]) -> Dict[str, Any]:
    ...
    node = normalize_metadata_node_record(normalize_endpoint_node_record(normalize_node_record(node)))
    return {
        **node,
...
def _sanitize_relationship_record(rel: Dict[str, Any]) -> Dict[str, Any]:
    ...
    identity_payload = {
        "schema_label": str(rel.get("schema_label") or rel.get("type") or ""),
        "start": {"label": start.get("label"), "key": start.get("key")},
        "end": {"label": end.get("label"), "key": end.get("key")},
        "props": props,
    }
    relationship_id = str(
        rel.get("relationship_id")
        or rel.get("rel_id")
        or props.get("relationship_id")
        or hashlib.sha256(
...
MERGE (a)-[r:{rtype} {{pring_rel_id: row.pring_rel_id}}]->(b)
